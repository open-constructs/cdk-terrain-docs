---
title: Variables and Outputs - CDK for Terraform
...
<Warning>The synthesized Terraform configuration will contain any values that you pass directly to CDKTF constructs. This includes credentials and any other sensitive data provided as input for the `cdktf` application. If you plan to commit the generated `cdk.tf.json` to version control, use [input variables](/terraform/cdktf/concepts/variables-and-outputs#input-variables) for secrets instead.</Warning>
