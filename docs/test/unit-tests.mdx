---
title: Unit Tests - CDK for Terraform
...
<Note>CDK for Terraform v0.20 introduces support for a `Testing.synthHcl` function. However, that is not compatible with other assertions, like `toHaveResourceWithProperties` etc. and should not be used.</Note>
