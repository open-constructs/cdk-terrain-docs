---
title: Refactoring - CDK Terrain
sidebarTitle: Refactoring Stacks
description: >-
  Refactor stacks as your CDKTN application grows in size and complexity.
---

You may need to move resources between stacks as your CDK Terrain (CDKTN) application grows in size and complexity. This guide demonstrates how to refactor your stacks. In general you can change the names of stacks freely without an effect on the synthesized code. If you change the name of a resource this might lead to a re-creation of the resource, especially if you move the resource from one stack to another.
...
When you run a `cdktn plan`, the CLI may display the following output:
...
Terraform (or OpenTofu) plans to destroy the old bucket and create a new one to replace it. The replacement can be harmful if there is already state in the resource, such as files in the bucket or data in the database.
...
Resource addresses in the context of nested constructs will not simply be the specified id given to the resource. When dealing with moving resources by their address to/from nested constructs, run `cdktn synth` and refer to `cdktf.out/stacks/'your stack name'` to find the exact address to move to/from.
...
When you run a `cdktn deploy '*'`, the CLI displays the following output.
...
```shell-session
$ cdktn synth # To ensure our synthesized stack is up to date
...
We can verify everything worked as expected by running a `cdktn deploy '*'` again. We should now see no changes.
...
This extra level adds a prefix to the name of the resource to ensure uniqueness. Refer to [Constructs](/concepts/constructs#scope) for more details.
