...
A stack represents a collection of infrastructure that CDK Terrain (CDKTN) synthesizes as a dedicated Terraform configuration. Stacks allow you to separate the state management for multiple environments within an application.

<Note>Stacks in CDKTN are different from the Terraform stacks concept announced at HashiConf 2023. Terraform stacks are a configuration layer that simplifies provisioning and managing resources at scale by controlling cross-configuration dependencies between Terraform modules. Refer to [Terraform stacks, explained](https://www.hashicorp.com/blog/terraform-stacks-explained) in the HashiCorp blog for additional information.</Note>
<Tip>Try the [Deploy Applications with CDK Terrain](/tutorials/deploy-applications) tutorial.</Tip>
...
The following example generates a single Terraform configuration in the configured output folder. When you run `cdktn synth`, the synthesized Terraform configuration will be in the folder `cdktf.out/stacks/a-single-stack`
...
<Tip>Try the [Deploy Multiple Lambda Functions with TypeScript](/tutorials/lambda-functions) tutorial. This tutorial guides you through a multi-stack application.</Tip>
...
Running `cdktn synth` produces the following synthesized stacks.
...
To deploy and destroy multiple stacks at once, either specify multiple stacks in the `cdktn deploy` and `cdktn destroy` command or use a wild card glob (e.g., `cdktn deploy '*-production'`).

Refer to [Best Practices](/create-and-deploy/best-practices#separate-business-units-with-stacks) for more details about when to create multiple stacks and how to structure them.
...
If you want to keep the result of the function attached to one stack and save its state, create a [Terraform Local value](/concepts/variables-and-outputs#local-values) and expose it. The following example creates a `TerraformLocal`.
...
<Warning>Escape hatches **must not** have empty arguments or objects, because CDKTN removes them from the synthesized JSON configuration.</Warning>
