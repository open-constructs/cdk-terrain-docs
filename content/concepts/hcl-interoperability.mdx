...
- In order to convert HCL to a CDKTN language, the [`cdktn convert`](/cli-reference/commands#convert) command can be used. It automatically translates HCL into a preferred CDKTN language. This is useful when working with an existing codebase that needs to be converted to CDKTN.
