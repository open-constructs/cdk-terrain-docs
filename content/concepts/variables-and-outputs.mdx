...
You can define [Terraform variables](/terraform/language/values/variables) as input parameters to customize [stacks](/concepts/stacks) and [modules](/concepts/modules). For example, rather than hardcoding the number and type of AWS EC2 instances to provision, you can define a variable that lets users change these parameters based on their needs.
