...
<Note>
  Enabling the flag on an existing project changes all asset hashes and paths once, so referencing resources see a one-time update. See [Canonical Asset Hashes](/release/canonical-asset-hashes) for details.
</Note>
