...
The following example uses a Data Source from the AWS Provider to fetch the Availability Zones of the given region. As this data is unknown until Terraform applies the configuration, this CDKTN application uses both [Terraform Outputs](/concepts/variables-and-outputs#output-values) and the Terraform [`element`](/terraform/language/functions/element) function.
