...
CDKTN [synthesizes](/cli-reference/commands#synth) infrastructure that you define in a supported programming language into [JSON configuration files](https://developer.hashicorp.com/terraform/language/syntax/json) that Terraform or OpenTofu can use to manage infrastructure.
...
A project declares the Terraform and OpenTofu version ranges it supports through the [`targetVersions`](/create-and-deploy/configuration-file#declare-target-runtimes) field in `cdktf.json`. These declared targets drive synth-time validations — for example [function availability](/concepts/functions#function-availability-across-terraform-and-opentofu) — without executing a binary. To additionally verify that the binary actually installed on a machine falls inside the declared range before `diff`, `deploy`, or `destroy` run it, opt in with [`validateInstalledBinary`](/create-and-deploy/configuration-file#validate-the-installed-binary).

import { WorkflowDiagram } from '/snippets/WorkflowDiagram.jsx';

<WorkflowDiagram />

CDKTN also automatically extracts the schemas from existing Terraform [providers](/concepts/providers) or [modules](/concepts/modules) and generates the necessary code bindings for your application.
...
- **CLI:** The [`cdktn` CLI](/cli-reference/cli-configuration) allows you to initialize a new CDKTN project, adjust project settings, synthesize your infrastructure into Terraform configuration files, deploy your CDKTN application, and more. Refer to the [CLI commands documentation](/cli-reference/commands) for more details. You can also use some Terraform CLI commands like `terraform apply` and `terraform destroy` directly, but we recommend using the available `cdktn cli` commands where possible. If you use OpenTofu, set `TERRAFORM_BINARY_NAME=tofu`. Refer to [Environment Variables](/create-and-deploy/environment-variables) for details.
...
Each CDKTN project has one or more `App` instances that act as a container for the infrastructure configurations you create and deploy. An `App` can have one or more [`Stacks`](/concepts/stacks) that represent a collection of related infrastructure.
...
Stacks allow you to separate the state management within an application. For example, you may want to deploy and manage separate infrastructure resources for development and testing. Reference the [stack documentation](/concepts/stacks) for more details on when and how to use stacks in your project.

### `Resource` Class

A `Resource` represents the definition for one or more infrastructure objects. Resources and their required attributes vary depending on the provider. Reference the [resources documentation](/concepts/resources) for more details.
...
Reference the [constructs documentation](/concepts/constructs) for more details.

### Examples

Reference the [examples page](/examples-and-guides/examples) to see configured CDKTN applications in each supported programming language.
