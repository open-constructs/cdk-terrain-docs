---
title: Configuration - CDK Terrain
...
export interface TerraformTargetVersions {
  readonly terraform?: string; // npm semver range of targeted Terraform versions, e.g. ">=1.5.7"
  readonly opentofu?: string; // npm semver range of targeted OpenTofu versions, e.g. ">=1.6.0"
}
...
export interface Config {
  ...
  readonly targetVersions?: TerraformTargetVersions; // Default: { terraform: ">=1.5.7", opentofu: ">=1.6.0" }. The Terraform-compatible runtimes the project intends to support
  readonly validateInstalledBinary?: boolean; // Default: false. Verify the installed Terraform/OpenTofu binary against targetVersions before commands execute it
}
```
...
CDK Terrain works with both Terraform and OpenTofu, which support slightly
different feature sets and introduce shared features at different versions.
The `targetVersions` field declares which runtimes — and which version ranges
of them — your project intends to support:
...
- Values are [npm semver ranges](https://github.com/npm/node-semver#ranges)
  (Terraform's `~>` constraint syntax is rejected, because npm semver would
  silently interpret it with different semantics — use `~1.5.7` or
  `">=1.5.7 <2"` instead).
- Omitting a product means your project does not target it. Omitting the
  whole field is equivalent to the default above: targeting every supported
  release of both products, the strictest and most portable claim.
- Synth-time validations check features against the **entire declared
  range**: declaring `terraform: ">=1.5.7"` while using a feature introduced
  in Terraform 1.8 fails validation, because the project claims to support
  versions where the feature is missing. Raise the declared floor (e.g.
  `">=1.8.0"`) to use the feature.
- These validations are deterministic and never execute a local binary, so
  they behave identically in CI, package builds, and environments without
  Terraform or OpenTofu installed.

Declared targets state your project's intent — they are not proof that the
binary installed on any given machine matches them. To verify the installed
binary explicitly, opt in with:
...
Commands that execute the Terraform-compatible CLI (`diff`, `deploy`,
`destroy`) will then fail early if the installed binary's product or version
falls outside the declared `targetVersions` (honoring the
`TERRAFORM_BINARY_NAME` environment variable for OpenTofu setups).
